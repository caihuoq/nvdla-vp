CPU = {
    ...
    extra_arguments = "-machine virt -cpu cortex-a57 -machine dumpdtb=virt.dtb -nographic -smp 1 -m 1024 -kernel /workspace/buildroot-2022.02.1/output/images/Image --append \"root=/dev/vda\" -drive file=/workspace/buildroot-2022.02.1/output/images/rootfs.ext4,if=none,format=raw,id=hd0 -device virtio-blk-device,drive=hd0 -fsdev local,id=r,path=.,security_model=none -device virtio-9p-device,fsdev=r,mount_tag=r -netdev user,id=user0,hostfwd=tcp::6666-:6666 -device virtio-net-device,netdev=user0"
}
