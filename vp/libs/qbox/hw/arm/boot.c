...
static void arm_load_kernel_notify(Notifier *notifier, void *data)
{
    ...
    if (arm_feature(&cpu->env, ARM_FEATURE_AARCH64) && kernel_size < 0) {
        kernel_size = load_aarch64_image(info->kernel_filename,
                                         info->loader_start + 0x2030000, &entry);
        //kernel_size = load_aarch64_image(info->kernel_filename,
        //                                 info->loader_start, &entry);
	printf("+++ info arch64 %s "TARGET_FMT_plx" \n", info->kernel_filename, info->loader_start);
        is_linux = 1;
    } else if (kernel_size < 0) {
        /* 32-bit ARM */
        entry = info->loader_start + KERNEL_LOAD_ADDR;
	printf("+++ info arch32 "TARGET_FMT_plx" \n", entry);
	...
	printf("+++ info bootloader "TARGET_FMT_plx" \n", info->loader_start);
        write_bootloader("bootloader", info->loader_start,
                         primary_loader, fixupcontext);
        //write_bootloader("bootloader", info->loader_start - 0x30000000,
        //                 primary_loader, fixupcontext);
